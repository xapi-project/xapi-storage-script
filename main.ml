...
let vdi_of_volume x =
  let ty = match List.Assoc.find x.Storage.Volume.Types.keys _vdi_type_key with
    | None -> "";
    | Some v -> v in
  let open Storage_interface in {
...
let update_keys root_dir name dbg sr key value response =
  ...
  | Some value ->
     set root_dir name dbg sr response.Storage.Volume.Types.key key value >>= fun () ->
...
let process root_dir name x =
  ...
  | { R.name = "SR.attach"; R.params = [ args ] } ->
    ...
    | Some (_, uri) ->
      let args' = Storage.Volume.Types.SR.Attach.In.make args.Args.SR.Attach.dbg uuid uri in
